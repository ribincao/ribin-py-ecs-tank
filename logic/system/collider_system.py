...
class ColliderSystem(System):
    ...
    def __init__(self, context: Context):
        ...
        self.box_collider_group: Group = self.context.get_group(Matcher("box2d_collider"))
        ...
        for i in range(len(entities)):
            ...
            for j in range(len(entities)):
                ...
                if entity_a.bullet and entity_b.model:
                    if entity_b.bullet:
                        continue
                    if entity_b.state and entity_b.model.model_name in ["wall", "enemy"]:
                        entity_b.state.state = State.destroy
                    if entity_b.model.model_index in ["symbol"]:
                        entity_b.model.model_index = "destroy"
                    if entity_a.state and entity_b.model.model_name not in ["water"]:
